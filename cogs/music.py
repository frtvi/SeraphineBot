...
class TutorialButton(discord.ui.View):
    def __init__(self):
        ...
        botaourl = discord.ui.Button(label="Meu github",url="https://github.com/frtvi")
...
class music(commands.Cog):
    ...
    async def help(self,interaction:discord.Interaction):
        ...
        helptxt = "`/ajuda` - Abre isso aqui.\n`/play` - Toca qualquer música do YouTube.\n`/fila` - Mostra a fila atual\n`/pular` - Pula a música."
    ...
    @app_commands.command(name="play",description="Toca qualquer música do YouTube.")
    @app_commands.describe(
        busca = "Digite a música do youtube"
    )
    async def play(self, interaction:discord.Interaction,busca:str):
        ...
        try:
            ...
            embedvc = discord.Embed(
                colour= 1646116,#grey
                description = 'Para tocar uma música, entra no canal de voz!'
            )
            ...
            if type(song) == type(True):
                embedvc = discord.Embed(
                    colour= 12255232,#red
                    description = 'Algo deu errado! Tente novamente (lembrando que não pego playlists e nem links do spotify!'
                )
                await interaction.followup.send(embed=embedvc)
            else:
                embedvc = discord.Embed(
    ...
    async def q(self, interaction:discord.Interaction):
        ...
        if retval != "":
            ...
            embedvc = discord.Embed(
                colour= 1646116,
                description = 'Não tem nenhuma música!'
            )
            await interaction.followup.send(embed=embedvc)
    ...
    async def pular(self, interaction:discord.Interaction):
        ...
        if self.vc != "" and self.vc:
            ...
            embedvc = discord.Embed(
                colour= 1646116,#grey
                description = f"Talvez você queira ouvir depois..."
            )
            await interaction.followup.send(embed=embedvc)
